...
local function SendTrain(Train,stationAdress,TrainType,origAdress)
	...
	while not result do
		...
		if not (stri==nil) then
			...
			if (Reply1=="Sent") then 
				m.send(address, tonumber(port), address.."TrainSent")
...
function RequestTrainLoop(TrainType,stationAdress,origAdress)
	local train
	...
	print("Sending Train")
	SendTrain(train,stationAdress,TrainType,origAdress)
end
...
local function MSGForUs(MSGSplit,msg,from,port)
	...
	if (FuncStri == "RequestTrain") then
		...
		RequestTrainLoop(TrainType,stationAdress,from)
	end
	if (FuncStri == "R") then
